...
<div class="wrap">
	...
	<fieldset>
        ...
        <p>
            <label>
                <input name="ss_display_choice" type="radio" value="regular_btn" class="tog" <?php echo get_option( 'ss_display_choice' ) === 'regular_btn' ? 'checked' : ''; // WPCS: XSS.EscapeOutput OK. ?> />
                Show a button that forwards the user to my SuperSaaS calendar
            </label>
        </p>
        <p>
            <label>
                <input name="ss_display_choice" type="radio" value="frame" class="tog" <?php echo get_option( 'ss_display_choice' ) === 'frame' ? 'checked' : ''; // WPCS: XSS.EscapeOutput OK. ?> />
                Show the calendar inside a small frame on my Wordpress site
            </label>
        </p>
        <p>
            <label>
                <input name="ss_display_choice" type="radio" value="popup_btn" class="tog" <?php echo get_option( 'ss_display_choice' ) === 'popup_btn' ? 'checked' : ''; // WPCS: XSS.EscapeOutput OK. ?> />
                Show a button that displays a popup on my site containing the calendar
            </label>
        </p>
    </fieldset>
    ...
    <p>
        <label>
            <input type="checkbox" name="ss_autologin_enabled" value="1" <?php echo get_option( 'ss_autologin_enabled' ) === '1' ? 'checked' : ''; // WPCS: XSS.EscapeOutput OK. ?> />
            If the user is logged into Wordpress, log him into your SuperSaaS account with the Wordpress user name
        </label>

        <br/>
        Automatically logging in the user requires your API key: <input type="text" name="ss_password" value="<?php echo get_option( 'ss_password' ); // WPCS: XSS.EscapeOutput OK. ?>" />
    </p>

    <p>
        Paste your button script here:
        <br/>
        <textarea name="ss_widget_script" rows="4" cols="80" placeholder="Paste the script generated by https://supersaas.com/info/doc/integration/integration_with_widget" >
