...
function supersaas_button_hook( $atts ) {
	...
	if ( $account && $password && $after ) {
		...
		if ( $current_user->ID ) {
			...
			if ( $image ) {
				$out .= '<input type="image" src="' . $image . '" alt="' . htmlspecialchars( $label ) . '" name="submit" onclick="confirmBooking"/>';
			} else {
				$out .= '<input type="submit" value="' . htmlspecialchars( $label ) . '" onclick="confirmBooking"/>';
			}
			...
			$out .= _e( 'Your username is a supersaas reserved word. You might not be able to login. Do you want to continue?', 'supersaas' ) . "');}}}</script>"; // WPCS: XSS.EscapeOutput OK.
		} else {
			$out = '';
		}
	} else {
